      ...
      <div className="py-14 sm:py-16">
        ...
        <div className="flex flex-wrap justify-center gap-3 pb-6 sm:pb-8">
          {filters.map((filter, idx) => (
            <Button
              ...
              className={`border border-zinc-300 px-4 py-3 rounded-xl text-sm sm:text-base flex items-center ${
                filter.active
        ...
        <div className="flex flex-wrap justify-center gap-4 pb-6 sm:pb-10 px-0 lg:px-32">
          {wordLists.map((word, i) => (
            <button
              key={i}
              className="bg-violet-50 text-indigo-950 font-medium px-4 sm:px-8 py-4 rounded-xl text-sm sm:text-base tracking-3 flex items-center last:w-full last:mx-4 sm:last:w-auto sm:last:mx-0"
            >
              {word}
            </button>
          ))}
